...
/**
 * Copy and download toolbar for transcripts.
 *
 * Renders two dropdown menus:
 * - Copy: plain text, markdown, segments JSON, or full response JSON to clipboard.
 * - Download: .txt, .md, or .json file.
 *
...
export default function TranscriptToolbar({ transcript }: Props) {
  ...
  /** Copy text to clipboard and show a 2-second confirmation. */
  ...
  function renderCopyMenu() {
    return (
      ...
      >
        ...
        <MenuItem
          onClick={() => handleCopy(toSegmentsJson(transcript.segments))}
        >
          Segments (JSON)
        </MenuItem>
  ...
  /** Render the download dropdown menu. */
  function renderDownloadMenu() {
    return (
      ...
      >
        <MenuItem
          onClick={() =>
            downloadFile(toPlainText(transcript), "transcript.txt")
          }
        >
          Plain text (.txt)
        </MenuItem>
